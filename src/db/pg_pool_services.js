const pool = require("./pg_init").instancePool.getPoolInstance();
const cOut = require("../utils/cOut")

const pgDBInit = () => {
    cOut.blue("   ---> pg driver correctly installed")
}
    ...
    return new Promise ( (resolve, reject) => {
        pool.connect( async (error_connection, client, release) => {
            try {
                ...
                console.log("error en try de dbPoolQuery", error.code, error.message);
                reject(error);
            } finally {
                release();
            }
